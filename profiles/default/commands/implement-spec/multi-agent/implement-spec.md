...
- `argus/specs/[this-spec]/tasks.md`
- `argus/roles/implementers.yml`

Create `argus/specs/[this-spec]/planning/task-assignments.yml` with this structure:
...
- In `.claude/agents/argus` there must be a file named by this implementer ID.

### PHASE 2: Delegate task groups implementations to assigned subagents

Loop through each task group in `argus/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned subagent specified in `task-assignments.yml`.

For each delegation, provide the subagent with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `argus/specs/[this-spec]/spec.md`
- Instruct subagent to:
  1. Perform their implementation
  2. Check off the task and sub-task(s) in `argus/specs/[this-spec]/tasks.md`
  3. Document their work in an implementation report named and numbered by this task name and placed in `argus/specs/[this-spec]/implementation/`.
...
3. If there are verifier roles, ensure those verifiers are defined in `argus/roles/verifiers.yml`.
...
4. If there are verifier roles, delegate to each verifier subagent:
   ...
   - Provide to the verifier:
     ...
     2. The spec file: `argus/specs/[this-spec]/spec.md` for context.
   - Instruct the verifier:
     ...
     3. Verify whether `argus/specs/[this-spec]/tasks.md` has been updated to reflect these tasks' completeness.
     4. Document your verification report and place this document in: `argus/specs/[this-spec]/verification/`
...
- The path to this spec: `argus/specs/[this-spec]`
Instruct the subagent to do the following:
  1. Run all of its final verifications according to its built-in workflow
  2. Produce the final verification report in `argus/specs/[this-spec]/verifications/final-verification.md`.
