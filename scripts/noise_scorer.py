    ...
    def score_findings(self, findings: List[Finding]) -> List[Finding]:
        ...
        for finding in findings:
            ...
            finding.noise_score = (
                0.4 * pattern_noise +
                0.4 * ml_noise +
                0.2 * (1.0 - finding.historical_fix_rate)
            )
...
def main():
    ...
    parser.add_argument('--history', default='.agent-os/finding_history.jsonl', 
                       help='Historical findings database')
    parser.add_argument('--update-history', action='store_true',
                       help='Update historical database with these findings')
