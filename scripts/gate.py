...
def main():
    parser = argparse.ArgumentParser(
        description="Agent-OS Policy Gate - Deterministic security gates"
    )
    parser.add_argument(
        "--stage",
        required=True,
        choices=["pr", "release"],
        help="Gate stage: pr or release"
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        "--sbom-present",
        action="store_true",
        help="SBOM is present (release only)"
    )
    parser.add_argument(
        "--signature-verified",
        action="store_true",
        help="Signature is verified (release only)"
    )
    parser.add_argument(
        "--provenance-present",
        action="store_true",
        help="SLSA provenance is present (release only)"
    )
    parser.add_argument(
        "--policy-dir",
        default="policy/rego",
        help="Directory containing Rego policies"
    )
