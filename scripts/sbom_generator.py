...
class SBOMGenerator:
    ...
    def _enrich_sbom(self, sbom: Dict, path: str) -> Dict:
        ...
        sbom['metadata']['tools'] = [{
            'vendor': 'Agent-OS',
            'name': 'Agent-OS Security Control Plane',
            'version': '1.0.0'
        }, {
            'vendor': 'Anchore',
            'name': 'Syft',
            'version': self._get_syft_version()
        }]
...
def main():
    ...
    parser.add_argument(
        'path',
        help='Directory or image to scan'
    )
    parser.add_argument(
        '-o', '--output',
    ...
    parser.add_argument(
        '--version',
        help='Release version (for release SBOMs)'
    )
    parser.add_argument(
        '--validate-only',
        action='store_true',
    ...
    if args.validate_only:
        ...
        # Generate new SBOM
        if args.version:
            output_file = generator.generate_for_release(
                args.path, 
                args.version,
                output_dir='sboms'
            )
