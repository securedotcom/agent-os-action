...
class LLMSecretDetector:
    ...
    def cross_validate(self, llm_findings: List[Finding], 
                      gitleaks_findings: List[Finding],
                      trufflehog_findings: List[Finding]) -> List[Finding]:
        """
        ...
        trufflehog_verified = {
            (f.path, f.line) for f in trufflehog_findings 
            if f.secret_verified == 'true'
        }
