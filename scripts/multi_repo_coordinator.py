...
class MultiRepoCoordinator:
    ...
    async def scan_repos(self, configs: List[ScanConfig]) -> List[ScanResult]:
        ...
        for i, task in enumerate(asyncio.as_completed(tasks), 1):
            ...
            print(f"{status_emoji} [{i}/{len(configs)}] {result.repo_name}: {result.status} "
    ...
    async def _clone_repo(self, repo_url: str, target_dir: str, branch: str) -> bool:
        ...
        try:
            ...
            _, stderr = await asyncio.wait_for(
                proc.communicate(),
                timeout=120  # 2 min timeout for clone
            )
    ...
    async def _scan_secrets(self, repo_path: str) -> List[Dict]:
        ...
        try:
            cmd = [
                ...
                repo_path,
                '--json',
                '--no-verification'  # Fast mode, verify later
            ]
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(
        description='Multi-repository security scanner coordinator'
    )
    parser.add_argument(
        'config',
        help='Path to repos config JSON file'
    )
    parser.add_argument(
        '--concurrent',
        type=int,
