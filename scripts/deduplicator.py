...
class FindingDeduplicator:
    ...
    def _merge_findings(self, findings: list[dict]) -> dict:
        ...
        sorted_findings = sorted(findings, key=lambda f: f.get("timestamp", "9999-99-99"))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Deduplicate findings across multiple repositories")
    parser.add_argument("findings_dir", help="Directory containing per-repo findings JSON files")
    parser.add_argument(
        "-o", "--output", default="deduplicated_findings.json", help="Output file for deduplicated findings"
    )
    parser.add_argument(
        "--duplicate-report", default="duplicate_report.json", help="Output file for duplicate groups report"
    )
    parser.add_argument(
        "--fuzzy-threshold", type=float, default=0.9, help="Similarity threshold for fuzzy matching (0.0-1.0)"
    )
