...
class TrivyScanner:
    ...
    def __init__(
        self,
        foundation_sec_enabled: bool = False,
    ...
    def _verify_trivy_installation(self) -> None:
        ...
        try:
            ...
            logger.error("   curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh")
...
The CWE is CWE-"""
            ...
            if hasattr(self.foundation_sec_model, 'generate'):
                cwe = self.foundation_sec_model.generate(
                    prompt,
                    max_new_tokens=10,
                    temperature=0.1
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Trivy CVE Scanner with Foundation-Sec-8B enrichment'
    )
    parser.add_argument(
        'target',
        help='Target to scan (directory path or container image)'
    )
    parser.add_argument(
        '--scan-type',
    ...
    parser.add_argument(
        '--foundation-sec',
        action='store_true',
        help='Enable Foundation-Sec-8B CWE mapping'
    )
    ...
    scanner = TrivyScanner(
        foundation_sec_enabled=args.foundation_sec,
    ...
    if args.scan_type == 'filesystem':
        ...
        result = scanner.scan_container_image(
            args.target,
            severity=args.severity,
            output_file=args.output
        )
